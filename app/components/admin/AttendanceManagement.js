    ...
    if(this.state.raidweeks.length > 0) {
      ...
      selectWeekdayRadios = (
        <RadioGroup className='radio' name='weekday' selectedValue={this.state.selectWeekday} onChange={value => AttendanceManagementActions.updateSelectWeekday(value)}>
          <div className={classNames( {'disabled': !currentRaidWeek.sunday})}><label><Radio value='sunday' disabled={!currentRaidWeek.sunday ? true : false}/>Sunday</label></div>
          <div className={classNames( {'disabled': !currentRaidWeek.monday})}><label><Radio value='monday' disabled={!currentRaidWeek.monday ? true : false}/>Monday</label></div>
          <div className={classNames( {'disabled': !currentRaidWeek.tuesday})}><label><Radio value='tuesday' disabled={!currentRaidWeek.tuesday ? true : false}/>Tuesday</label></div>
          <div className={classNames( {'disabled': !currentRaidWeek.wednesday})}><label><Radio value='wednesday' disabled={!currentRaidWeek.wednesday ? true : false}/>Wednesday</label></div>
          <div className={classNames( {'disabled': !currentRaidWeek.thursday})}><label><Radio value='thursday' disabled={!currentRaidWeek.thursday ? true : false}/>Thursday</label></div>
          <div className={classNames( {'disabled': !currentRaidWeek.friday})}><label><Radio value='friday' disabled={!currentRaidWeek.friday ? true : false}/>Friday</label></div>
          <div className={classNames( {'disabled': !currentRaidWeek.saturday})}><label><Radio value='saturday' disabled={!currentRaidWeek.saturday ? true : false}/>Saturday</label></div>
        </RadioGroup>
      )
    }
